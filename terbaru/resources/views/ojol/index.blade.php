...
<div class="content-header clearfix">
  <ol class="breadcrumb">
    <li><a href="/item">Produk</a></li><!-- react-text: 754 -->
    <!-- /react-text -->
    <li><span class="current">Daftar Harga Ojek Online</span></li>
  </ol>
  <div class="content-header-action pull-right">

    <button type="button" class="btn btn-primary" style="cursor:pointer" data-toggle="modal"
...
<div class="panel">
  <div class="panel-heading">
    <h3 class="panel-title"> Daftar Harga Ojek Online</h3>
  </div>
  ...
  <div class="panel-body">
    <table class="table table-hover">
      ...
      <tbody>
        @foreach($ojol as $ojol)
        <tr>
          <td>{{$ojol->nama}} </td>
          <td>{{$ojol->deskripsi}} </td>
          <td>{{$ojol->status}} </td>
          <td>
            {{ $ojol->catatProduks->count() }}
          </td>
          <td>
            @if($loggedIn->role_id == 1 || $loggedIn->role_id == 2)
            <a href="/ojol/{{$ojol->id}}/edit" class="btn btn-warning btn-sm">Edit</a>
            <a href="/ojol/{{$ojol->id}}/delete" class="btn btn-danger btn-sm"
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Data Daftar Harga Ojek Online</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="/ojol/create" method="POST" enctype="multipart/form-data">
          {{csrf_field()}}
